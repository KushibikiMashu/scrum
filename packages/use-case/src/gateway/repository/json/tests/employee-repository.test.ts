...
//
//     await expect(repository.save(employee))
//       .rejects.toThrowError('社員IDはnullである必要があります。社員の更新は update メソッドを使ってください')
//   })
// })
...
describe('update', () => {
  ...
  test('新規の社員を更新したとき、例外を投げる', async () => {
    ...
    await expect(repository.update(employee))
      .rejects.toThrowError('値が null です')
  })
})
...
describe('delete', () => {
  ...
  test('存在しない社員を削除しようとすると例外を投げる', async () => {
    ...
    await expect(repository.delete(employee))
      .rejects.toThrowError('社員ID 800 は存在しません')
  })
});
